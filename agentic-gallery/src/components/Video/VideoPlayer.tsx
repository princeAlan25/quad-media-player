...
const VideoPlayerContent: React.FC<VideoPlayerProps> = ({ initialVideoUrl }) => {
  ...
  const { videos, addVideos: addVideosToLibrary, removeVideo: moveVideoToTrash } = useMediaLibrary();
  ...
  return (
    ...
    >
      <div
        ...
        className={`w-full h-full transition-transform duration-500 ease-in-out ${slideDirection === 'left' ? 'slide-in-left' : 'slide-in-right'}`}
      >
        <video
          ref={videoRef}
          ...
          {videos.map((v, idx) => (
            ...
            >
              <span
                ...
                className="absolute top-1 right-1 p-1 rounded-full bg-black/60 text-white/80 hover:bg-red-600 hover:text-white transition-colors"
                title="Move to trash"
              >
